...
MultivariateNormalDensity = function(X, mu = rep(0,length(X)), Sigma = 1, log = TRUE){
  if (length(Sigma) != 1){
    emdbook::dmvnorm(x = X, mu = mu, Sigma = Sigma, log = log)
  }
  else{
    if (log) {
...
Sigma2FCDSample <- function(N = 1, Y, X, betas, segment_indexes,
  ...
  for (h in 1:max(segment_indexes)) {
    ...
    #}
      ...
      # ---------- STABLE COLLAPSED SIGMA^2 FCD ----------
      
      D <- diag(1 / delta2, k)
      A <- crossprod(X_current) + D + ridge * diag(k)
      M <- crossprod(X_current, Y_current) + D %*% mu_betas[[h]]
      
      # Solve A beta_hat = M via Cholesky (fallback to ginv)
      chol_ok <- TRUE
      R <- tryCatch(
        chol(A),
        error = function(e) {
      ...
      if (chol_ok) {
        beta_hat <- backsolve(R, forwardsolve(t(R), M))
      } else {
        message("Cholesky failed in Sigma2FCDSample, segment ", h,
                " — using pseudo-inverse")
        beta_hat <- ginv(A) %*% M
      }
      
      # Numerically stable quadratic form
      res <- Y_current - X_current %*% beta_hat
      quad <- as.numeric(crossprod(res)) +
        as.numeric(crossprod(beta_hat - mu_betas[[h]],
                             D %*% (beta_hat - mu_betas[[h]])))
...
Delta2FCDSample <- function(N = 1, betas_list, segment_indexes,
  ...
  InverseGammaSample(N = N, a = alpha + 0.5 * k * max(segment_indexes), b = beta + 0.5 * total)
}
...
LogMarginal <- function(Y, X, delta2 = 1, mu_beta = rep(0, ncol(X)),
  ...
  E <- sum(sapply(1:max(segment_indexes), function(h) {
    ...
    res <- Yh - Xh %*% mu_beta[[h]]
    Xt_res <- t(Xh) %*% res
    sum(res^2) - delta2 * t(Xt_res) %*% inv_Ah %*% Xt_res
  }))
  E <- max(E,0)
  as.numeric(lgamma(N / 2 + a) - lgamma(a) - N / 2 * log(pi) +
               a * log(2 * b) - D / 2 - (N / 2 + a) * log(2 * b + E))
  #as.numeric(
  #  lgamma(N / 2 + a) - lgamma(a) - N / 2 * log(2 * pi) +
  #    a * log(b) - D / 2 - (N / 2 + a) * log(b + E / 2)
  #)
}
